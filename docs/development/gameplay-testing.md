...
selected partitions. The residual workspace job keeps all other non-map packages
...
PR #180 added one other explicit target, `hex_game/tests/spell_resolution.rs`, for the
cross-crate spell transaction.
...
discovery, the target remains explicitly registered and runs as the ordinary
`contracts` postflight; its owning rules, trajectory, ECS, and real-map seams also run
in their normal concerns.
...
### Spell-resolution evidence after PR #180

PR #180 temporarily carried a narrow review concern while its spell-resolution work
was still in flight. That delivery-only routing has been retired now that the wave is
on `dev`; it is not a reusable selector tier or precedent for future changes.

The lasting regression evidence is owned by the ordinary fail-closed concern graph.
Trajectory and volume rules remain in `trajectory_contracts`; rules, ECS, map seam,
and application consumers run through their normal owning concerns; and the explicit
renderer-free `hex_game/tests/spell_resolution.rs` target remains covered by the
ordinary `contracts` postflight. Unknown paths, selector changes, and pushes to
`dev` or `main` select the complete gate. PR #180's exact historical evidence remains
recorded in its delivery plan and audit trail.

Screenshots may judge the presentation of a hook-established casting state, but the
gameplay hooks, snapshots, and renderer-free contracts remain authoritative for
payment, impact ordering, settlement, and authority release.
