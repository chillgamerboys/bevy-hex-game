...
| Contract | Publisher | Consumer | Status | Specified in |
|---|---|---|---|---|
| `SimulationRole::{Authority, Replica}` / `AuthoritativeSystems` | core | authority-owning runtime systems | **partial** — gameplay AI/reducers/movement are gated and replica presentation remains live; world mutation/perception gates wait on L3 | [client-hosted sandbox manifest](planning/waves/client-hosted-sandbox/manifest.md) |
...
| `CommandRequestId`, seatless `LocalGameCommandRequest`, and seatless wire `GameCommandRequest` | local/network ingress | authenticated authority adapter | **live** — offline, listen-host, and replica input converge on authenticated ingress; the host derives seats and emits idempotent typed outcomes | [authority map](planning/waves/client-hosted-sandbox/maps/authority-and-command.md) |
...
| `SessionManifestV1`, random `SessionInstanceId`, `LobbySnapshot`, redacted invite/reconnect credentials, `CommandResult` | shared session authority | host/client adapters and UI | **partial** — admission, lobby, sequencing, endpoint/SPKI/expiry/session-bound credential persistence, and direct transport are live; L4 presentation/adapters remain | [L1 order](planning/waves/client-hosted-sandbox/orders/L1-session-runtime.md) |
| `UnitReplica` / `SessionReplica` | gameplay authority | remote ECS/UI adapters | **partial** — gameplay authority publishes/applies disclosure-safe projections; world disclosure withdrawal waits on L3 | [world/disclosure map](planning/waves/client-hosted-sandbox/maps/world-and-disclosure.md) |
| `WorldSnapshotV1`, `PublicWorldFingerprintV1`, `WorldDeltaV1`, `PlayerKnowledgeSnapshotV1`, `LiveSessionSnapshotV1` | world and perception authority through shared stable-name data contracts | reconnect/import and authorized replica adapters | **agreed** — bounded canonical shared protocol is live and temporarily world-ratified; L3 must implement transactional export/import, current fingerprinting, deltas, and disclosure withdrawal without exposing private plans or hostile knowledge | [2026-08-10 amendment](planning/waves/client-hosted-sandbox/manifest.md#2026-08-10--temporary-world-authority-and-generator-neutral-restore-contract) |
| Default-off `MultiplayerPlugin` with Replicon/Aeronet/WebTransport | shared composition | explicit Host Direct / Join Direct runtime | **partial** — custom auth, SPKI-pinned direct transport, in-memory composition, and socket-free default are live; L4 supplies player-facing endpoint orchestration | [L1 order](planning/waves/client-hosted-sandbox/orders/L1-session-runtime.md) |
