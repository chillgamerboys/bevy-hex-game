...
| `TerrainImpact { batch, volume, ElementId, power }` — declarative canonical-volume voxel damage | gameplay | world | **partial** — #175 map admission/resolution is live on `dev`; draft #180 adds the paid gameplay spell publisher and monotonic batch ledger, but is not live until the wave merges | [planning/boundary.md](planning/boundary.md) G |
| `TerrainImpactOutcome` — one applied or rejected answer with exact per-voxel health transitions | world | gameplay | **partial** — #175 map publication and #178 exhaustive answer validation are live on `dev`; draft #180 adds correlation, authority hold, settlement, and release | [planning/boundary.md](planning/boundary.md) H |
...
| `TerrainSystems` — `ApplyWorld → RefreshProjections → ReconcileActors → ConsumeOutcomes` before perception and later combat authority | core | world / gameplay | **partial** — #175 map `ApplyWorld` and #178 shared ordering are live on `dev`; draft #180 wires occupancy/movement refresh, deterministic actor settlement and authority adoption, then matching-outcome consumption | [planning/boundary.md](planning/boundary.md) H |
