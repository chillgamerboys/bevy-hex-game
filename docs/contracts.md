...
| `WorldSnapshotV1`, `CurrentWorldSnapshotV1`, `PublicWorldFingerprintV1`, and transactional `WorldReplicationRequestV1` / result — canonical stable-name voxels, partial health, and every public semantic/presentation consequence needed to reproduce the same `TerrainReady` world | world | multiplayer reconnect and future Campaign restore | **live** — export, teardown/import, canonical deltas, mutation refresh, bounds, and complete public-fingerprint verification are map-owned and generator-neutral | [planning/boundary.md](planning/boundary.md#d2--world-snapshot-the-live-generator-proof-restore-format) |
...
| Contract | Publisher | Consumer | Status | Specified in |
|---|---|---|---|---|
| `SimulationRole::{Authority, Replica}` / `AuthoritativeSystems` | core | authority-owning runtime systems | **live** — AI, reducers, movement, encounter/world mutation, perception, and saving remain authority-only while replica presentation/network application stays active | [client-hosted sandbox manifest](planning/waves/client-hosted-sandbox/manifest.md) |
...
| `CommandRequestId`, seatless `LocalGameCommandRequest`, and seatless wire `GameCommandRequest` | local/network ingress | authenticated authority adapter | **live** — offline, listen-host, and replica input converge on authenticated ingress; the host derives seats and emits idempotent typed outcomes | [client-hosted sandbox manifest](planning/waves/client-hosted-sandbox/manifest.md) |
| Seatless `ClientLobbyRequest`; local-only `HostSessionControlRequest`; `SessionControlResult` | client/listen-host UI adapters | shared session authority | **live** — remote readiness/leave and host assignment/kick/launch/outcome/retry/lobby/close converge on one authority; host-only actions are absent from wire registration | [client-hosted sandbox manifest](planning/waves/client-hosted-sandbox/manifest.md) |
| `SessionManifestV1`, random `SessionInstanceId`, `LobbySnapshot`, redacted invite/reconnect credentials, `CommandResult` | shared session authority | host/client adapters and UI | **live** — admission, six-seat assignment/readiness, sequencing, endpoint/SPKI/expiry/session-bound credential persistence, reconnect, and typed close/refusal presentation share one canonical session authority | [client-hosted sandbox manifest](planning/waves/client-hosted-sandbox/manifest.md) |
| `UnitReplica` / `SessionReplica` | gameplay authority | remote ECS/UI adapters | **live** — gameplay publishes and applies exact disclosure-safe projections; newly observed actors materialize as replica shells and withdrawal fully removes hidden hostiles without exporting lattice or combat authority | [client-hosted sandbox manifest](planning/waves/client-hosted-sandbox/manifest.md) |
| `WorldSnapshotV1`, `PublicWorldFingerprintV1`, `WorldDeltaV1`, `PlayerKnowledgeSnapshotV1`, `LiveSessionSnapshotV1` | world and perception authority through shared stable-name data contracts | reconnect/import and authorized replica adapters | **live** — bounded transactional world restore, complete current fingerprinting, ordered idempotent deltas, player-memory hydration, and disclosure withdrawal are composed without private plans, hostile knowledge, or `CombatState` | [2026-08-10 amendment](planning/waves/client-hosted-sandbox/manifest.md#2026-08-10--temporary-world-authority-and-generator-neutral-restore-contract) |
| `HostCampaignCheckpointV2` | shared stable checkpoint vocabulary; world/gameplay adapters remain owner-specific | host-owned Campaign persistence only | **live** — complete world, unit, lattice, effect-ledger, formation, rules/seeds, and active-time state is bounded, transport-neutral, transactionally restored, and written only by a listen host at the quiescent paused-exploration boundary | [host-owned Campaign manifest](planning/waves/host-owned-campaign/manifest.md) |
| `SessionTransportKind`, `OnlineIdentityState`, seatless `OnlineSessionRequest`, typed `OnlineSessionEvent`, redacted `OnlineJoinCode`/`OnlineLobbyId`/`OnlinePrincipal`, and platform presentation fields | shared protocol | future safe online adapter and UI | **foundation** — store-neutral vocabulary, 80-bit Crockford code validation/digesting, secret redaction, and default-off mock composition are live; no EOS identity, lobby, or P2P call is active yet | [EOS feasibility foundation](planning/eos-feasibility-foundation.md) |
| `SnapshotTransferHeaderV1`, `SnapshotChunkV1`, and `SnapshotTransferProgress` | shared protocol | future EOS reconnect transfer lane | **foundation** — 32 KiB chunks, eight-chunk window vocabulary, 64 MiB bounds, baseline/hash metadata, and validation are fixed; zstd streaming and Aeronet transfer behavior remain the Online wave | [EOS feasibility foundation](planning/eos-feasibility-foundation.md) |
| Default-off `MultiplayerPlugin` with Replicon/Aeronet/WebTransport and opt-in DNS-SD | shared composition | explicit Direct or same-LAN host/join runtime | **live** — custom auth, audited SPKI-pinned direct transport, player-facing endpoint/lobby orchestration, open-lobby-only LAN advertisement/discovery, macOS native-Bonjour publication confirmation, and socket-free ordinary startup ship together; discovery never bypasses admission | [LAN testing runbook](development/lan-multiplayer-testing.md) |
| Default-off `OnlinePlugin` and isolated `hex_eos_ffi` loader | shared online infrastructure | future `Play Online` composition | **foundation** — source builds do not load EOS or open sockets; only an explicit absolute staged runtime is accepted and absence produces typed refusal. Protected SDK ABI and live sandbox proof remain required before Online dispatch | [EOS feasibility foundation](planning/eos-feasibility-foundation.md) |
