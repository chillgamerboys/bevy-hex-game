...
To try Worm, run `python3 tools/arena.py launch --map fort --encounter worm` from the repository and press **Enter**. **Esc/Tab** pauses, **Shift+R** resets, and the existing movement/charge controls remain. Choose **Spectate Battle** on the ready screen to watch two monster teams.
