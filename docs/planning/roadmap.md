...
| Crystal Mountain and cross-biome tunnel | **In Progress / draft PR #210:** refreshed from `dev` at `495a73d` with identity selection and the three Crystal required-ignored identities retained. The old `74deb7f` checks and captures are historical; current-head all-platform CI (including macOS), complete acceptance, and exact-head Crystal presentation/motion/control/play approval remain before #211 | map / presentation |
