...
| Grand V3 schematic and map compiler | **Proxy performance checkpoint / approval pending:** the approved revision-2 planner now feeds an additive V3 `Schematic` layout at pitch 22. The representative radius-187 proxy proves 105,469 exact columns, 217 stable biome identities, 444 fixed resident chunks, chunk-local terrain replacement, unchanged snapshot wire shape, complete lifecycle publication, and deterministic teardown/re-entry. Headless setup/build are below current Crystal Mountain, but the undecorated one-entity-per-run renderer publishes 421,876 terrain entities and reaches 1.72 GB RSS / 2.86 GB macOS peak footprint. Approve budgets and the recommended combined-terrain-chunk-mesh plus exact-picking lane before final hydrology, routes, Crystal integration, vegetation, bridges, full review, and delivery to `dev`; no scale or fidelity reduction is proposed | map / presentation |
