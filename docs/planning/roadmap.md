...
| Grand V3 schematic planner | **Review-ready:** a pure deterministic radius-eight planner and CLI now resolve all 217 layered coarse cells, preserve exact authored landmarks and networks, constrain bounded biome envelopes, vary independently seeded coastline/hydrology/islands/landforms/woodland, emit strict RON and semantic fingerprints, and atomically publish diagnostic SVG/HTML galleries. The 256- and 10,000-seed gates pass; Linux scheduled peak-memory evidence, human approval, and delivery to `dev` remain. Voxel terrain compilation and scale/palette experiments follow as separate phases | map |
