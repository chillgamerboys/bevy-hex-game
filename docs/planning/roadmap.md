...
| Grand V3 schematic planner | **Automated correction complete / visual approval pending:** the pure deterministic radius-eight planner and CLI now use the approved revision-2 cell-for-cell trace. Flat-top orientation, twelve north-eastern peaks, mountain-lake formation, frozen-woods contact, waterfall opening, straight Crystal tunnel, and land-overlay river route are exact and locked across seeds. Package, 256-seed, and 10,000-seed release gates pass; a fresh reference and twelve-seed gallery replace every stale revision-1 artifact. Named-human gallery review, authoritative Linux peak-memory evidence, repository integration, and delivery to `dev` remain. Voxel terrain compilation and scale/palette experiments follow separately | map |
