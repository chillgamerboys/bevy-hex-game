...
| Character-first camera | adaptive same-yaw terrain collision, whole-tree fading, ordinary opaque cave roofs, and seed-exact multi-azimuth traversal over every selectable map and Two Rings region are implemented; remaining work is human motion/readability approval and a later product rule limiting Map mode to appropriate scenarios | world / presentation |
