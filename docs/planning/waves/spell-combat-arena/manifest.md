...
1. First person default; C toggles tightly constrained third person. WASD, mouse look, Space jump, Shift sprint, 1/2/3 select. All spells release on mouse-up; Shield/Fireball gain launch speed over a 0.75-second hold and Area Blast stays fixed. T toggles selected projectile preview. Enter or Start begins from a frozen ready screen. Escape/Tab pauses and releases the mouse; no live HUD menu button. The paused menu includes Resume, Reset, Fullscreen/Windowed, Quit, and tuning. Shift+R fully resets to the ready screen.
