...
Active continuation, September 12, 2026. **Keep implementing until complete or live Codex quota reaches 3% remaining.** The reset was confirmed; `waiting_for_reset=false`. Latest usage check: 82% remaining. Earlier checkpoints are preserved in Git history rather than repeated as current instructions here.

## Candidate and authoritative requirements

- Integration checkout: `/Users/alberto/Documents/Codex/2026-09-11/i-w/work/hex-expedition`, branch `wave/forest-expedition`. Current integrated runtime head is **3a0dd3f**; the following documentation commit records these results. No remote writes or dev/main merges. Preserve source branches and the old playable `work/hex-forest` checkout/launcher until composed acceptance.
...
1. All agents have released their targets. Root will capture six repair views (full overview, both bridge arches, charged forest/mountain fountains and spent fountain) from a clean committed candidate with package a538. Inspect every full-resolution original plus contact sheet independently. Keep source/package frozen across the pack.
